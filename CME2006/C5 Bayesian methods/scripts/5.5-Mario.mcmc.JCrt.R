...
# created by Mario dos Reis (https://www.qmul.ac.uk/sbbs/staff/mariodosreisbarros.html)
# modified by Sishuo Wang as to the window sizes
...
r.lnprior <- function(r) log(dexp(r,1))
#t.lnprior <- function(t, mu=22.45, sd=5.6) -((t-mu)/sd)^2  # constant deleted
t.lnprior <- function(t) log(dexp(t,1/0.15))
lnL.rt <- function(r, t, x, n) lnL(d=2*r*t, x=x, n=n)
...
N <- 1e6
for(w in seq(0.1,1,0.2)){
    rt.2 <- rt.mcmcf(1, 1, N, w, w)
    rt.2 <- lapply(rt.2, function(x){ x[round(length(x)/2):N] } )
    cat(w, sapply(rt.2, mean), "\n")
}
